...
export function getGradeShort(grade: Grade | string): string {
...
export const WHATNOT_FEE_RATE = 0.08; // Whatnot standard seller fee

export function netAfterFee(grossValue: number, feeRate = WHATNOT_FEE_RATE): number {
  return grossValue * (1 - feeRate);
}
...
export function calculateNetROI(costBasis: number, grossSaleValue: number, feeRate = WHATNOT_FEE_RATE): number {
  if (costBasis === 0) return 0;
  const net = netAfterFee(grossSaleValue, feeRate);
  return ((net - costBasis) / costBasis) * 100;
}
